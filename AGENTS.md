...
- 列表页默认采用轻量、专业、后台操作型表格风格。优先让表格承载业务信息，不要用大面积装饰卡片或重标题制造层级。
- 当页面需要展示数量、进度或状态汇总时，优先把数字放在对应列表区块标题旁，例如 `待确认淘汰 2 头`、`确认淘汰 1 头`。如果这些数字已经能被列表标题表达，不再额外放置统计卡、进度条卡片或重复的 KPI 卡。
- 页面标题、卡片标题、列表标题都要克制：使用中等字号和中等字重，避免整页多层标题都加粗导致视觉抢占。层级优先通过间距、浅色说明文字、细边框、表格分组来表达。
- 同一页面中不要重复展示同一个状态数量。例如 `确认淘汰` 数量如果已经出现在列表标题，就不要再出现在顶部 tab、数据卡或进度条里。
- 按用户当前要完成的工作拆分列表，而不是按后台全生命周期堆状态。比如淘汰决策页应展示 `待确认淘汰` 与 `确认淘汰` 两个列表，不展示 `已淘汰`、出售、转移、死亡、离场等后续执行状态。
- 表格视觉要轻：浅色表头、细分隔线、紧凑行高、稳定列宽、统一排序/筛选图标位置。风险值、ID、选中态可以适度强调，普通字段保持安静。
- 操作按钮应贴近对应列表区块。批量操作按钮优先放在待处理列表的右下或工具栏区域，不要放在远离表格的独立卡片中。
...
- 列表页与对应详情页必须双向保持字段一致：如果新增、删除、重命名、拆分、合并或调整列表字段，必须检查对应详情页并同步相同业务字段变化；如果修改详情页字段，也必须检查对应列表页并同步相同业务字段变化。列表中的拆分/聚合展示可以因展示位置调整，例如列表合并展示、详情拆分展示，但不得修改字段名称、业务含义、可见性口径或遗漏对应字段。若列表与详情因明确业务场景需要故意不同，必须在 PRD 或修改日志中说明原因，并在最终回复中提示用户。
...
- 同一用户动作的入口按钮、弹窗标题、页面标题、确认按钮和返回路径必须使用统一文案；不要出现按钮叫 `创建疫苗计划`、弹窗标题却叫 `新增免疫计划` 这类同义但不同名的情况。
