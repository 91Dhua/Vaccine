...
这些结果通过任务详情主页中的 `淘汰与留种进度` 模块进入查看。该模块放在 `断奶记录` 下方，并展示 `标记留种 / 计划留种` 进度。数字区域只负责展示数量和 hover 口径说明，详情跳转入口放在进度标题旁的 `查看` 图标上。

- 用户点击 `留种进度` 标题旁的 `查看` 图标后，跳转进入二级 `留种仔猪` 页面。
- 二级页面顶部需要继续展示该进度详情，并在下方展示 `育种公猪 / 育种母猪` 名单及来源信息。
- `育种公猪 / 育种母猪` 列表至少包含：仔猪ID、母猪ID、父系ID、位置、日龄、断奶体重。
