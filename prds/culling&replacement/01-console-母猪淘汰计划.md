...
这些结果可以通过批次里的 `任务详情` 入口查看。任务详情主页中，`断奶记录` 下方需要独立放置一个 `淘汰与留种进度` 模块，其中展示 `已淘汰 / 计划淘汰` 与 `标记留种 / 计划留种` 两条进度，并提示用户点击进度条可查看对应详情。

- 用户点击 `已淘汰 / 计划淘汰` 进度条后，跳转进入二级 `淘汰猪只` 页面。
- 二级页面顶部需要继续展示该进度详情，并在下方展开淘汰结果汇总与淘汰猪只列表。

- `淘汰结果汇总`：计划淘汰、确认淘汰、已淘汰、建议淘汰但未淘汰
- `淘汰猪只列表`：统一单表展示本轮所有进入淘汰决策的母猪，包括管理者建议淘汰对象和现场新增需淘汰对象
...
- 来源（建议淘汰 / 现场新增 / 无）
- 不淘汰原因
- 后续去向（未处理 / 售卖 / 转移 / 场内屠宰 / 死亡）

列表采用单表结构，不按状态拆成多个 tab；通过顶部轻量筛选切换 `全部 / 仅看确认淘汰 / 仅看已淘汰 / 仅看建议淘汰但未淘汰`。
