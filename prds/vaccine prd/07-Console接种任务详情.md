...
flowchart TD
  ...
  F --> K{点击详情页操作?}
  K -->|补充接种| L[自动带入未接种猪只与原任务接种信息并进入第二步配置]
  K -->|重新接种| M[若已安排补打则二次确认，再带入剩余目标猪只进入第二步配置]
```
...
| 疫苗任务信息 | 采用上下排版，单列展示任务编号、任务状态、疫苗、品牌、接种方式、剂量、接种日期、目标范围、创建人/时间；任务进入进行中或已检测后，额外展示操作人/时间；`已接种 / 目标接种` 通过进度条旁的数值统一展示 | 读取任务快照与执行聚合 |
...
| 免疫复核结果 | 放在免疫复核设置下方；仅在实验室已提交结果后展示；汇总模块与复核结果详情页保持一致，采用上方 `检测进度` 概览、下方指标卡片的结构，展示当前合格率、阳性/阴性数量、阈值与检测结果 | 读取关联免疫复核采样任务回写的结果快照 |
...
| 已接种列表 | 不展示状态字段，但需要展示操作人/时间，便于复盘每头猪的执行记录 |
| 猪只列表 | 分为 `已接种`、`未接种` 两类；其中 `未接种` 的字段与 `待接种` 状态保持一致 |
| 免疫复核设置 | 若该任务开启了复核，则继续回显相同复核配置，不因任务状态改变展示口径 |
| 免疫复核结果 | 若复核结果已回录，则与复核结果详情页汇总模块保持一致，展示检测进度、当前合格率、阳性/阴性数量、阈值与合格/不合格判定 |
...
| 接种猪只列表 | 分为 `已接种`、`未接种` 两类；`已接种` 列表继续展示操作人/时间 |
| 免疫复核设置 | 若该任务开启了复核，则继续回显相同复核配置，不额外改写成系统推导文案 |
| 免疫复核结果 | 若复核结果已回录，则与复核结果详情页汇总模块保持一致，展示检测进度、当前合格率、阳性/阴性数量、阈值与最终判定，供管理者复盘 |
| 补充接种 | 当存在未接种猪只且 `补打状态 = 需补打` 时，仅在接种猪只列表的 `未接种` Tab 右上角展示 `补充接种` 按钮，`已接种` Tab 不展示；未接种分组支持勾选猪只，默认选中全部未接种猪只，用户可只选择部分猪只补充接种；若用户已安排整批重新接种，则该按钮置灰并显示问号说明 |
| 重新接种 | 当实验室回写的免疫复核结果为不合格，且 `补打状态 = 需补打` 时，在 `免疫复核结果` 卡片右上角展示 `重新接种` 按钮 |
| 补充接种链路 | 自动沿用原任务的接种方式、疫苗、品牌、剂量、剂量单位与用户勾选的未接种猪只，进入第二步配置页；疫苗、品牌、接种方式、剂量、剂量单位预填并锁定，接种日期留空且需要用户手动填写；第三步按钮文案为 `完成补打` |
| 重新接种链路 | 自动沿用原任务的目标猪群、接种方式、疫苗、品牌、剂量、剂量单位，进入第二步配置页；页面标题为 `重新接种`，疫苗、品牌、接种方式、剂量、剂量单位预填并锁定，接种日期留空且需要用户手动填写；第三步按钮文案为 `确认接种`；若已有补打任务，则先弹二次确认并排除已安排补打的猪只 |
| 编辑限制 | 不允许修改任务配置，不允许删除任务 |
...
| 已检测任务存在未接种猪只 | 详情页展示接种猪只列表中的未接种分组，并允许用户勾选部分或全部未接种猪只后通过补充接种创建一条新任务 |
